...
function getSteamDetails(g){const sid=g.steamId?String(g.steamId):"";const fromCat=g.steamDetails||null;const fromCache=sid&&typeof STEAM_DETAILS_CACHE!=="undefined"?STEAM_DETAILS_CACHE[sid]:null;if(fromCache&&fromCat)return{description:fromCache.description||fromCat.description||"",min:fromCache.min||fromCat.min||"",rec:fromCache.rec||fromCat.rec||""};return fromCache||fromCat||null;}
function modalFallbackShots(g){const sid=g.steamId?String(g.steamId):"";const urls=[];const push=u=>{if(u&&!isBrokenCoverUrl(u)&&!urls.includes(u))urls.push(u);};push(g.heroImg);push(g.img);if(sid){push(steamHeroCover(sid));push(steamLibraryCover(sid));push("https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/"+sid+"/library_hero_2x.jpg");}return urls;}
function renderImagesHtml(g){const sid=g.steamId?String(g.steamId):"";const cacheReady=typeof STEAM_ABOUT_CACHE!=="undefined";if(sid&&!cacheReady)return"<p class=\"modal-hint\">Cargando imágenes…</p>";const about=sid&&cacheReady?STEAM_ABOUT_CACHE[sid]:null;let shots=about?.screenshots?[...about.screenshots]:[];if(!shots.length)shots=modalFallbackShots(g);if(!shots.length)return"<p style=\"color:var(--muted)\">Sin capturas de gameplay disponibles.</p>";return`<div class="desc-images-grid">${shots.map(url=>descShotHtml(url,g.name)).join("")}</div>`;}
...
function normalizeCatalog(){catalog.forEach((g,i)=>{g._order=i;if(!Array.isArray(g.tags))g.tags=[];(g.delivery||[]).forEach(d=>{if(d.id==="code"){d.name="CD Key";d.desc="Activás en la plataforma indicada en cada edición (Steam, Rockstar, EA, Ubisoft…).";}if(d.id==="account"){d.name="Cuenta";d.desc="Cuenta lista con el juego en la plataforma de esa edición.";}});if(!g.icon||g.icon==="game")g.icon="🎮";if(/Red Dead Redemption 2/i.test(g.name)){g.platform="Rockstar / Steam - PC";g.description="Clave Ultimate: activación en Rockstar Games Launcher (no Steam). Cuentas: Steam.";const cd=g.delivery.find(x=>x.id==="code");if(cd)cd.desc="Clave para Rockstar Games Launcher / Social Club. No es código de Steam.";}});}
...
function isBrokenCoverUrl(u){const s=String(u||"").toLowerCase();if(!s)return true;return /fakepng/.test(s)||(/encrypted-tbn0\.gstatic|gstatic\.com\/images\?q=tbn/.test(s)&&/[&?]s=\d{1,2}\b/.test(s))||(/\/thumb\//.test(s)&&!/steamstatic\.com/.test(s)&&!/steamgriddb/.test(s));}
...
function steamHeroCover(sid){return sid?"https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/"+sid+"/library_hero_2x.jpg":"";}
function gameCardCover(g){const sid=g.steamId?String(g.steamId):"";const img=g.img||"";if(img&&!isBrokenCoverUrl(img)){if(/library_600x900|steamgriddb|\.webp$/i.test(img))return img;if(/header/i.test(img)&&g.heroImg&&!isBrokenCoverUrl(g.heroImg)&&/library_hero/i.test(g.heroImg))return g.heroImg;return img;}return steamLibraryCover(sid)||"";}
function gameHeroCover(g){const sid=g.steamId?String(g.steamId):"";const custom=g.heroImg||g.img||"";if(custom&&!isBrokenCoverUrl(custom))return custom;return steamHeroCover(sid)||"";}
function coverImgMarkup(g){const src=gameCardCover(g);const icon=gameIcon(g);if(!src)return `<span class="cover-fallback-icon">${icon}</span>`;const alt=g.name.replace(/"/g,"&quot;");const isAnim=/\.webp$/i.test(src);const parts=[];if(g.heroImg&&g.heroImg!==src)parts.push(g.heroImg);if(g.img&&g.img!==src)parts.push(g.img);if(g.steamId){const sid=g.steamId;parts.push(steamLibraryCover(sid));parts.push("https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/"+sid+"/library_600x900_2x.jpg");parts.push(steamHeroCover(sid));}const fb=parts.filter((u,i,a)=>u&&u!==src&&!isBrokenCoverUrl(u)&&a.indexOf(u)===i).join("|");return `<img class="cover-img${isAnim?" cover-img-anim":""}" src="${src}"${fb?` data-fb="${fb.replace(/"/g,"&quot;")}"`:""} data-icon="${icon}" alt="${alt}" width="600" height="900" loading="lazy" decoding="async" onerror="coverImgFallback(this)">`;}
