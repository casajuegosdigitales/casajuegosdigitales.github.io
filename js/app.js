...
function isBrokenCoverUrl(u){const s=String(u||"").toLowerCase();if(!s)return true;return /fakepng|encrypted-tbn0\.gstatic|gstatic\.com\/images\?q=tbn/.test(s)||(/\/thumb\//.test(s)&&!/steamstatic\.com/.test(s)&&!/steamgriddb/.test(s));}
...
function coverImgMarkup(g){const src=gameCardCover(g);const icon=gameIcon(g);if(!src)return `<span class="cover-fallback-icon">${icon}</span>`;const alt=g.name.replace(/"/g,"&quot;");const parts=[];if(g.heroImg&&g.heroImg!==src)parts.push(g.heroImg);if(g.img&&g.img!==src)parts.push(g.img);if(g.steamId){const sid=g.steamId;parts.push(steamLibraryCover(sid));parts.push("https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/"+sid+"/library_600x900_2x.jpg");parts.push(steamHeroCover(sid));}const fb=parts.filter((u,i,a)=>u&&u!==src&&!isBrokenCoverUrl(u)&&a.indexOf(u)===i).join("|");return `<img class="cover-img" src="${src}"${fb?` data-fb="${fb.replace(/"/g,"&quot;")}"`:""} data-icon="${icon}" alt="${alt}" width="600" height="900" loading="lazy" decoding="async" onerror="coverImgFallback(this)">`;}
