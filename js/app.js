...
function steamAppCoverUrl(sid,kind){if(!sid)return"";const base="https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/"+sid+"/";return kind==="hero"?base+"library_hero_2x.jpg":base+"library_600x900_2x.jpg";}
function steamCoverBelongsToApp(url,sid){return sid&&String(url||"").includes("/apps/"+sid+"/");}
function pickSteamCoverUrl(g,preferVertical){const sid=g.steamId?String(g.steamId):"";const pool=[g.img,g.heroImg].filter(Boolean);const ok=pool.filter(u=>!isUntrustedCoverUrl(u)&&(!sid||steamCoverBelongsToApp(u,sid)));if(preferVertical){const vert=ok.find(u=>/library_600x900/i.test(u));if(vert)return vert;}const header=ok.find(u=>/header/i.test(u)||/header_alt/i.test(u));if(header)return header;if(ok.length)return ok[0];return sid?steamAppCoverUrl(sid,"card"):"";}
function gameCardCover(g){return pickSteamCoverUrl(g,true)||"";}
function gameHeroCover(g){return pickSteamCoverUrl(g,false)||(g.steamId?steamAppCoverUrl(g.steamId,"hero"):"");}
function coverImgMarkup(g){const src=gameCardCover(g);const icon=gameIcon(g);if(!src)return `<span class="cover-fallback-icon">${icon}</span>`;const alt=g.name.replace(/"/g,"&quot;");const parts=[];const sid=g.steamId?String(g.steamId):"";if(g.img&&g.img!==src)parts.push(g.img);if(g.heroImg&&g.heroImg!==src)parts.push(g.heroImg);if(sid){parts.push(steamAppCoverUrl(sid,"card"));parts.push("https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/"+sid+"/library_600x900_2x.jpg");parts.push(steamAppCoverUrl(sid,"hero"));}const fb=parts.filter((u,i,a)=>u&&u!==src&&!isUntrustedCoverUrl(u)&&a.indexOf(u)===i).join("|");return `<img class="cover-img" src="${src}"${fb?` data-fb="${fb.replace(/"/g,"&quot;")}"`:""} data-icon="${icon}" alt="${alt}" width="600" height="900" loading="lazy" decoding="async" onerror="coverImgFallback(this)">`;}
