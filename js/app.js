...
function getSteamDetails(g){const sid=g.steamId?String(g.steamId):"";if(sid&&typeof STEAM_DETAILS_CACHE!=="undefined"&&STEAM_DETAILS_CACHE[sid])return STEAM_DETAILS_CACHE[sid];return null;}
...
function renderImagesHtml(g){const sid=g.steamId?String(g.steamId):"";const about=sid&&typeof STEAM_ABOUT_CACHE!=="undefined"?STEAM_ABOUT_CACHE[sid]:null;let shots=about?.screenshots?[...about.screenshots]:[];if(!shots.length&&g.steamId){shots=[`https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/${g.steamId}/header.jpg`];}if(!shots.length){const hero=gameHeroCover(g);if(hero)shots=[hero];}if(!shots.length)return"<p style=\"color:var(--muted)\">Sin imágenes disponibles.</p>";return`<div class="desc-images-grid">${shots.slice(0,3).map(url=>descShotHtml(url,g.name)).join("")}</div>`;}
function renderInfoTabs(g){const tabs=[{id:"desc",label:"Imágenes"},{id:"req",label:"Requisitos"},{id:"guide",label:"Guía"}];document.getElementById("mInfoTabs").innerHTML=tabs.map(t=>`<button type="button" class="info-tab ${state.infoTab===t.id?"active":""}" onclick="selInfoTab('${t.id}')">${t.label}</button>`).join("");const panel=document.getElementById("mInfoPanel");if(!state.infoTab){panel.innerHTML="<p class=\"modal-hint\">Tocá <strong>Imágenes</strong>, <strong>Requisitos</strong> o <strong>Guía</strong>.</p>";return;}const sd=getSteamDetails(g);const esc=s=>String(s||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");if(state.infoTab==="desc"){if(typeof STEAM_ABOUT_CACHE==="undefined"){panel.innerHTML="<p class=\"modal-hint\">Cargando imágenes…</p>";return;}panel.innerHTML=renderImagesHtml(g);}else if(state.infoTab==="req"){if(typeof STEAM_DETAILS_CACHE==="undefined"){panel.innerHTML="<p class=\"modal-hint\">Cargando requisitos…</p>";return;}const r=sd?{min:sd.min,rec:sd.rec}:{};if(!r.min&&!r.rec)panel.innerHTML="<p class=\"modal-hint\">Sin requisitos cargados.</p>";else panel.innerHTML=`<div class="req-grid">${renderRequirementsCol("Mínimos",r.min,esc)}${renderRequirementsCol("Recomendados",r.rec,esc)}</div>`;}else{if(!state.delivery)panel.innerHTML="<p class=\"modal-hint\">Elegí <strong>CD Key</strong> o <strong>Cuenta</strong> arriba.</p>";else{const selV=state.version?g.versions.find(x=>x.id===state.version):null;panel.innerHTML=getActivationGuide(g,state.delivery,selV);}}}
function selInfoTab(id){state.infoTab=id;const g=catalog.find(x=>x.id===state.game);const done=()=>renderInfoTabs(g);if(id==="desc")ensureSteamAboutCache().then(done).catch(()=>{const panel=document.getElementById("mInfoPanel");if(panel)panel.innerHTML="<p class=\"modal-hint\">No se pudieron cargar las imágenes.</p>";});else if(id==="req")ensureSteamDetailsCache().then(done).catch(()=>{const panel=document.getElementById("mInfoPanel");if(panel)panel.innerHTML="<p class=\"modal-hint\">No se pudieron cargar los requisitos.</p>";});else done();}
...
function openModal(id){const g=catalog.find(x=>x.id===id);state={game:id,delivery:null,version:null,payment:null,step:0,infoTab:null};renderModalHeader(g);renderModal();applyModalBg();document.getElementById("modalOverlay").classList.add("open");document.body.style.overflow="hidden";}
...
function initHeroSpotlight(){
  ...
  el.innerHTML=HERO_SPOTLIGHT_IDS.map(id=>{
    ...
    const eager=HERO_SPOTLIGHT_IDS.indexOf(id)===0;
