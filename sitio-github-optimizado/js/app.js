...
function getSteamDetails(g){const sid=g.steamId?String(g.steamId):"";if(sid&&typeof STEAM_DETAILS_CACHE!=="undefined"&&STEAM_DETAILS_CACHE[sid])return STEAM_DETAILS_CACHE[sid];return g.steamDetails||null;}
...
function minGamePricing(g){let best=null;g.versions.forEach(v=>{const c=priceCuotas(v);if(!best||c<best.cuotas)best={cuotas:c,transfer:priceTransfer(v)};});return best;}
...
function versionsForDelivery(g,delId){const f=g.versions.filter(v=>!v.deliveryType||v.deliveryType===delId);return f.length?f:g.versions;}
...
function normalizeCatalog(){catalog.forEach((g,i)=>{g._order=i;g.delivery.forEach(d=>{if(d.id==="code"){d.name="🔑 Clave digital";d.desc="Activás en la plataforma indicada en cada edición (Steam, Rockstar, EA, Ubisoft…).";}if(d.id==="account"){d.name="👤 Cuenta con el juego";d.desc="Cuenta lista con el juego en la plataforma de esa edición.";}});if(!g.icon||g.icon==="game")g.icon="🎮";if(/Red Dead Redemption 2/i.test(g.name)){g.platform="Rockstar / Steam - PC";g.description="Clave Ultimate: activación en Rockstar Games Launcher (no Steam). Cuentas: Steam.";const cd=g.delivery.find(x=>x.id==="code");if(cd)cd.desc="Clave para Rockstar Games Launcher / Social Club. No es código de Steam.";}});}
...
let state={game:null,delivery:null,version:null,payment:"transfer",step:0,infoTab:"desc"};
...
function renderModalPricing(g){const p=state.delivery?minPricingForDelivery(g,state.delivery):minGamePricing(g);document.getElementById("mPriceFrom").innerHTML=`<div class="mpf-lbl">Precio desde</div><div class="mpf-row"><span class="modal-price-common">${fmt(p.cuotas)}</span><span class="modal-price-transfer">Transferencia: ${fmt(p.transfer)}</span></div>`;}
function renderDeliveryOptions(g){const needPulse=!state.delivery;const multi=g.delivery.length>1;const pulseCls=d=>needPulse&&multi?(d.id==="code"?" opt-pulse-code":d.id==="account"?" opt-pulse-account":""):"";const opts=g.delivery.map(d=>`<div class="opt ${state.delivery===d.id?"sel":""}${pulseCls(d)}" onclick="selD('${d.id}')"><div class="opt-t">${d.name}</div><div class="opt-d">${deliveryOptionDesc(g,d)}</div></div>`).join("");const note=state.delivery==="account"?`<div class="delivery-account-note">Te entregamos la cuenta con acceso al juego y al correo asociado para que puedas cambiar los datos y conservarla de forma permanente.</div>`:"";document.getElementById("mDelivery").innerHTML=opts;document.getElementById("mDelivery").className="modal-delivery-opts"+(needPulse?(multi?" has-both-pulse":" has-single-pulse"):"");const sec=document.querySelector(".modal-delivery-section");if(sec){sec.classList.toggle("needs-choice",!state.delivery);const old=sec.querySelector(".delivery-account-note");if(old)old.remove();if(note){const el=document.createElement("div");el.innerHTML=note;sec.appendChild(el.firstChild);}}}
function renderPlatformHint(g){const el=document.getElementById("mPlatformInfo");document.getElementById("mPlat").innerHTML=renderGamePlatformEl(g);if(!state.delivery){el.style.display="none";return;}const vers=versionsForDelivery(g,state.delivery);const plats=[...new Set(vers.map(v=>platformFromVersionName(v.name)))];let inner="";if(state.delivery==="code"){if(g.id===37)inner=platformRowHtml("rockstar",'La clave digital es para <strong>Rockstar Games Launcher</strong> (no Steam).');else if(plats.length===1)inner=platformRowHtml(plats[0],`La clave digital es para <strong>${platformLabel(plats[0])}</strong>.`);else inner=`<span class="plat-row-txt">La plataforma de cada clave (Steam, Rockstar, EA, Ubisoft…) figura en la edición.</span><div class="plat-logos-group">${plats.map(platformLogoHtml).join("")}</div>`;}else{if(plats.length===1)inner=platformRowHtml(plats[0],`La cuenta incluye el juego en <strong>${platformLabel(plats[0])}</strong>.`);else inner=`<span class="plat-row-txt">La plataforma de la cuenta figura en cada edición.</span><div class="plat-logos-group">${plats.map(platformLogoHtml).join("")}</div>`;}el.innerHTML=inner;el.style.display="block";}
...
function renderDescHtml(g){const esc=s=>String(s||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");const sid=g.steamId?String(g.steamId):"";const about=sid&&typeof STEAM_ABOUT_CACHE!=="undefined"?STEAM_ABOUT_CACHE[sid]:null;const sd=getSteamDetails(g);if(about&&(about.intro||(about.sections&&about.sections.length)||(about.screenshots&&about.screenshots.length))){const shots=about.screenshots||[];if(about.sections&&about.sections.length){const introText=pickDescText(about.intro,sd?.description,g.description);const intro=introText?`<p class="desc-intro">${esc(introText)}</p>`:"";const secs=about.sections.map((sec,i)=>{const t=sec.title?`<h4 class="desc-sec-h">${esc(cleanDescText(sec.title))}</h4>`:"";const ptxt=pickDescText(sec.text);const p=ptxt?`<p class="desc-sec-p">${esc(ptxt)}</p>`:"";const img=shots[i]?descShotHtml(shots[i],g.name):"";return`<section class="desc-sec">${t}${p}${img}</section>`;}).join("");return`<div class="desc-steam">${intro}${secs}</div>`;}const desc=pickDescText(about.intro,sd?.description,g.description);const intro=desc?`<p class="desc-intro">${esc(desc)}</p>`:"";const sents=desc.split(/(?<=[.!?])\s+/).filter(x=>x.trim().length>10);const max=Math.min(shots.length,5);let html=intro,si=0;for(let i=0;i<max;i++){const chunk=Math.max(1,Math.ceil((sents.length-si)/(max-i)));const block=sents.slice(si,si+chunk).join(" ");si+=chunk;if(block)html+=`<p class="desc-sec-p">${esc(block)}</p>`;html+=descShotHtml(shots[i],g.name);}return`<div class="desc-steam">${html}</div>`;}const desc=pickDescText(sd?.description,g.description)||"Sin descripción disponible.";const parts=desc.split(/(?<=[.!?])\s+/).filter(p=>p.trim().length>12);const body=parts.length>1?parts.map(p=>`<p class="desc-sec-p">${esc(p.trim())}</p>`).join(""):`<p class="desc-sec-p">${esc(desc)}</p>`;return`<div class="desc-steam desc-steam-fallback">${body}</div>`;}
function renderInfoTabs(g){const tabs=[{id:"desc",label:"Descripción"},{id:"req",label:"Requisitos"},{id:"guide",label:"Guía de activación"}];document.getElementById("mInfoTabs").innerHTML=tabs.map(t=>`<button type="button" class="info-tab ${state.infoTab===t.id?"active":""}" onclick="selInfoTab('${t.id}')">${t.label}</button>`).join("");const panel=document.getElementById("mInfoPanel");const sd=getSteamDetails(g);const esc=s=>String(s||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");if(state.infoTab==="desc"){panel.innerHTML=renderDescHtml(g);}else if(state.infoTab==="req"){const r=sd?{min:sd.min,rec:sd.rec}:g.requirements||{};if(!r.min&&!r.rec)panel.innerHTML="<p>Requisitos no especificados para este título.</p>";else panel.innerHTML=`<div class="req-grid">${renderRequirementsCol("⚙️ Mínimos",r.min,esc)}${renderRequirementsCol("🚀 Recomendados",r.rec,esc)}</div>`;}else{if(!state.delivery)panel.innerHTML="<p>Seleccioná <strong>Clave digital</strong> o <strong>Cuenta con el juego</strong> arriba para ver la guía de activación.</p>";else{const selV=state.version?g.versions.find(x=>x.id===state.version):null;panel.innerHTML=getActivationGuide(g,state.delivery,selV);}}}
function selInfoTab(id){state.infoTab=id;const g=catalog.find(x=>x.id===state.game);const done=()=>renderInfoTabs(g);if(id==="desc")ensureSteamAboutCache().then(done);else if(id==="req")ensureSteamDetailsCache().then(done);else done();}
...
function openModal(id){const g=catalog.find(x=>x.id===id);state={game:id,delivery:null,version:null,payment:"transfer",step:0,infoTab:"desc"};renderModalHeader(g);renderModal();document.getElementById("modalOverlay").classList.add("open");document.body.style.overflow="hidden";ensureSteamAboutCache().then(function(){if(state.game===id&&state.infoTab==="desc")renderInfoTabs(g);});ensureSteamDetailsCache().then(function(){if(state.game===id&&state.infoTab==="req")renderInfoTabs(g);});}
...
function renderModal(){const g=catalog.find(x=>x.id===state.game);renderDeliveryOptions(g);renderPlatformHint(g);renderModalPricing(g);if(state.infoTab==="guide")renderInfoTabs(g);const steps=modalSteps();const progHint=`<span class="progress-hint">Paso ${state.step+1} de ${steps.length}</span>`;document.getElementById("progress").innerHTML=progHint+steps.map((s,i)=>{const done=i<state.step,cur=i===state.step;return `<div class="pw"><div class="pr"><div class="pd ${done?"done":cur?"cur":""}">${done?"✓":i+1}</div>${i<steps.length-1?`<div class="pl ${done?"done":""}"></div>`:""}</div><div class="plbl ${done?"done":cur?"cur":""}">${s}</div></div>`;}).join("");let chips="";if(state.delivery){const d=g.delivery.find(x=>x.id===state.delivery);if(d)chips+=`<span class="sel-chip">${d.name}</span>`;}if(state.version){const v=g.versions.find(x=>x.id===state.version);chips+=`<span class="sel-chip" onclick="goStep(0,true)">${v.name}<span style="opacity:.6;font-size:10px"> ✕</span></span>`;}document.getElementById("selBar").innerHTML=chips;const sV=document.getElementById("sV"),sP=document.getElementById("sP"),sS=document.getElementById("sS");[sV,sP,sS].forEach(s=>s.classList.remove("active"));if(state.step===0){renderVersion(g);sV.classList.add("active");if(state.version){renderPayment(g,true);sP.classList.add("active");}else{sP.innerHTML="";}}else if(state.step===1){renderPayment(g,false);sP.classList.add("active");}else{renderSummary(g);sS.classList.add("active");}}
function goStep(s,reset){if(reset){state.payment="transfer";state.version=null;}state.step=s;renderModal();}
function renderVersion(g){if(!state.delivery){document.getElementById("sV").innerHTML=`<div class="step-lbl"><span class="sn">2</span>Elegí la versión</div><p style="font-size:12px;color:var(--muted);padding:10px 0">Primero seleccioná arriba si querés <strong>clave digital</strong> o <strong>cuenta con el juego</strong>.</p>`;return;}const vers=versionsForDelivery(g,state.delivery);document.getElementById("sV").innerHTML=`<div class="step-lbl"><span class="sn">2</span>Elegí la versión</div><div class="opts">${vers.map(v=>{const c=priceCuotas(v),t=priceTransfer(v);return`<div class="opt ${state.version===v.id?"sel":""}" onclick="selV('${v.id}')"><div class="opt-t">${v.name}</div>${versionPlatformHtml(v)}<div style="margin-top:7px"><span style="font-size:11px;color:#9a9ad4;text-decoration:line-through;display:block">${fmt(priceLista(c))}</span><span style="font-family:Orbitron,sans-serif;font-size:15px;font-weight:700;color:#fff">${fmt(c)}</span><div style="font-size:11px;color:var(--green);margin-top:4px;font-weight:600">Transferencia: ${fmt(t)}</div><div class="cuotas-shine" style="margin-top:6px;margin-bottom:0"><span class="cuotas-shine-text">6x sin interés</span><span class="cuotas-shine-val">${fmt(Math.round(c/6))}</span></div></div></div>`;}).join("")}</div>`;}
function renderPayment(g,inline){const v=g.versions.find(x=>x.id===state.version);if(!v){document.getElementById("sP").innerHTML="";return;}const cuotas=priceCuotas(v),transfer=priceTransfer(v);const backBtn=inline?"":`<button class="btn-back" onclick="prevStep()">← Volver</button>`;const nextFn=inline?"goToSummary()":"nextStep()";document.getElementById("sP").innerHTML=`${backBtn}<div class="step-lbl"><span class="sn">3</span>Método de pago</div><p style="font-size:11px;color:var(--text-soft);margin-bottom:12px"><strong style="color:var(--green)">Recomendado:</strong> transferencia o cripto con <strong style="color:var(--green)">30% de descuento</strong></p><div class="pay-opts">${payMethods.map(p=>{const fp=calcPrice(cuotas,p.id,transfer);const cs=p.cuotas>1?`${p.cuotas}x ${fmt(fp/p.cuotas)}`:"";const rec=(p.id==="transfer"||p.id==="cripto")?" pay-opt-recommended":"";const recTag=(p.id==="transfer"||p.id==="cripto")?`<span class="pay-rec-tag">Recomendado</span>`:"";return `<div class="pay-opt ${state.payment===p.id?"sel":""}${rec}" onclick="selP('${p.id}')">${recTag}<span class="pi">${p.icon}</span><div class="pn">${p.name}</div><div class="pnote ${p.noteClass}">${p.note}</div><div class="pp">${fmt(fp)}</div>${cs?`<div class="pc">${cs}</div>`:""}</div>`;}).join("")}</div><button class="btn-action" ${!state.payment?"disabled":""} onclick="${nextFn}">Ver resumen →</button>`;}
function renderSummary(g){const v=g.versions.find(x=>x.id===state.version);const p=payMethods.find(x=>x.id===state.payment);const d=g.delivery.find(x=>x.id===state.delivery)||g.delivery[0];const cuotas=priceCuotas(v),transfer=priceTransfer(v);const fp=calcPrice(cuotas,p.id,transfer);const diff=fp-cuotas;const cs=p.cuotas>1?`${p.cuotas} cuotas de ${fmt(fp/p.cuotas)} sin interés`:"";document.getElementById("sS").innerHTML=`<button class="btn-back" onclick="prevStep()">← Volver</button><div class="step-lbl"><span class="sn">✓</span>Resumen del pedido</div><div class="ps-box"><div class="psr"><span class="psl">Juego</span><span class="psv">${g.name}</span></div><div class="psr"><span class="psl">Entrega</span><span class="psv">${d.name}</span></div><div class="psr"><span class="psl">Versión</span><span class="psv">${v.name}</span></div><div class="psr"><span class="psl">Método de pago</span><span class="psv">${p.name}</span></div><div class="psr"><span class="psl">Precio tarjeta</span><span class="psv">${fmt(cuotas)}</span></div>${diff!==0?`<div class="psr"><span class="psl">Ajuste (${p.pct>0?"+":""}${p.pct}%)</span><span class="${diff<0?"ps-disc":"ps-sur"}">${diff<0?"-":"+"} ${fmt(Math.abs(diff))}</span></div>`:""}<hr class="ps-div"><div class="psr"><span class="ps-tl">Total a pagar</span><span class="ps-tv">${fmt(fp)}</span></div>${cs?`<div style="font-size:11px;color:var(--green);text-align:right;margin-top:3px;font-weight:600">${cs}</div>`:""}</div><div class="summary-actions"><button type="button" class="btn-wsp-confirm" onclick="confirmOrderWsp()">Confirmar pedido por WhatsApp</button><button type="button" class="btn-action-secondary" onclick="addToCart()">Agregar al carrito y seguir comprando</button><button type="button" class="btn-link-muted" onclick="closeModal()">Seguir viendo juegos</button><p class="order-legal">Producto digital · Entrega coordinada por WhatsApp</p></div>`;}
function selD(id){state.delivery=id;state.version=null;const g=catalog.find(x=>x.id===state.game);renderModalPricing(g);renderPlatformHint(g);if(state.infoTab==="guide")renderInfoTabs(g);renderModal();}
function selV(id){state.version=id;const g=catalog.find(x=>x.id===state.game);renderPlatformHint(g);if(state.infoTab==="guide")renderInfoTabs(g);renderModal();scrollToPayment();}
function selP(id){state.payment=id;renderModal();}
...
function updateTrustStats(){const n=typeof catalog!=="undefined"?catalog.length:68;const label=n>=60?"60+":String(n)+"+";document.querySelectorAll("[data-trust-count]").forEach(el=>{el.textContent=label;});}
...
function handleSearch(q){
  ...
  const ms=catalog.filter(g=>g.name.toLowerCase().includes(q.toLowerCase())).slice(0,6);
...
function makeCard(g){
  ...
  const sum=editionSummaryText(g);
  const opts=sum?`<span class="card-variants">${sum}</span>`:"";
  return `<div class="product-card" onclick="openModal(${g.id})">
    <div class="product-img">
      ...
      ${sum?`<span class="cover-editions-badge">${sum}</span>`:""}
    </div>
...
function renderCatalogPage(){
  ...
  grid.innerHTML=slice.length?slice.map(makeCard).join(""):'<div style="text-align:center;padding:5rem 2rem;color:var(--muted)"><div style="font-size:52px;margin-bottom:1rem;opacity:.4">&#128269;</div><p>No encontramos juegos con ese filtro.</p></div>';
  renderCatalogPager();
...
function applyFilters(){
  let r=catalog.filter(g=>activeTag==="all"||g.tags.includes(activeTag));
  ...
  filteredCatalog=r;catalogPage=1;renderCatalogPage();void(0:`<div style="text-align:center;padding:5rem 2rem;color:var(--muted)"><div style="font-size:52px;margin-bottom:1rem;opacity:.4">🔍</div><p>No encontramos juegos con ese filtro.</p></div>`;
...
const HERO_SPOTLIGHT_IDS=[48,39,7,12];
const HERO_SPOTLIGHT_LABELS={48:"The Last of Us Part II",39:"Resident Evil Requiem",7:"Black Myth: Wukong",12:"Crimson Desert"};
function initHeroSpotlight(){
  const el=document.getElementById("heroSpotlight");
  if(!el)return;
  el.innerHTML=HERO_SPOTLIGHT_IDS.map(id=>{
    const g=catalog.find(x=>x.id===id);
    if(!g)return "";
    const title=HERO_SPOTLIGHT_LABELS[id]||g.name;
    const img=gameCardCover(g);
    const safe=title.replace(/"/g,"&quot;");
    return '<a class="hero-promo-card" href="#" onclick="event.preventDefault();openModal('+g.id+')" aria-label="'+safe+'"><div class="hero-promo-cover">'+(img?'<img class="cover-img" src="'+img+'" sizes="(max-width:700px) 45vw, 300px" alt="'+safe+'" width="600" height="900" loading="eager" fetchpriority="high" decoding="async">':'<span class="cover-fallback-icon">'+gameIcon(g)+'</span>')+'</div></a>';
  }).join("");
}
function initHomeGrids(){
  const pick=(tag,n)=>catalog.filter(g=>g.tags.includes(tag)).slice(0,n);
  const mas=document.getElementById("masGrid");if(mas)mas.innerHTML=pick("masvendidos",8).map(makeCard).join("")||"<p style='color:var(--muted);grid-column:1/-1'>Proximamente mas juegos.</p>";
  const nuev=document.getElementById("nuevGrid");if(nuev)nuev.innerHTML=pick("nuevos",8).map(makeCard).join("");
  const of=document.getElementById("ofGrid");if(of)of.innerHTML=pick("ofertas",8).map(makeCard).join("");
}
